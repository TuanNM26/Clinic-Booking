...
export class EditColumnDate1745805810564 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `UPDATE "appointments" SET "reason_canceled" = '' WHERE "reason_canceled" IS NULL`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "doctor_shifts" ALTER COLUMN "date" DROP DEFAULT`,
    );
    await queryRunner.query(`ALTER TABLE "doctor_shifts" DROP COLUMN "status"`);
    await queryRunner.query(
      `CREATE TYPE "public"."doctor_shifts_status_enum" AS ENUM('active', 'cancelled')`,
    );
    await queryRunner.query(
      `ALTER TABLE "doctor_shifts" ADD "status" "public"."doctor_shifts_status_enum" NOT NULL DEFAULT 'active'`,
    );
    await queryRunner.query(
      `ALTER TABLE "appointments" ALTER COLUMN "reason_canceled" SET NOT NULL`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "appointments" ALTER COLUMN "start_time" SET DEFAULT '00:00:00'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "role_permissions" ADD "deletedBy" character varying`,
    );
    await queryRunner.query(
      `ALTER TABLE "role_permissions" ADD "deletedAt" TIMESTAMP`,
    );
    await queryRunner.query(
      `ALTER TABLE "role_permissions" ADD "updatedBy" character varying`,
    );
    await queryRunner.query(
      `ALTER TABLE "role_permissions" ADD "updatedAt" TIMESTAMP NOT NULL DEFAULT now()`,
    );
    await queryRunner.query(
      `ALTER TABLE "role_permissions" ADD "createdBy" character varying`,
    );
    await queryRunner.query(
